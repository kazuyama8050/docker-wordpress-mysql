		...
		getMember : function( email ) {
			...
			$.ajax( s ).done(function( data ){
				if(data.status_code === 'ok'){
					...
					alert( '<?php _e("The relevant membership information does not exist.", "usces"); ?>' );
				}else{
					alert( '<?php _e("Membership information could not be retrieved.", "usces"); ?>' );
				}
			}).fail(function( data ){
				alert( '<?php _e("Membership information could not be retrieved.", "usces"); ?>' );
			});
...
<?php if( $reduced_taxrate ): ?>
	<?php if( 'products' == $tax_target ):
		...
		<tr>
			<td colspan="5" class="aright"><?php echo $tax_label; ?><span id="include_tax"><?php echo $include_tax; ?></span></td>
