...
?>
		...
		change_product_search_field_0 :function (){
			...
			if( column == 'item_option' ) {
				...
				html = '<?php _e('option name', 'usces'); ?>:<input name="search[product_word][0]" type="text" value="<?php echo esc_attr($arr_search['product_word'][0]); ?>" class="text" maxlength="50" /> <?php _e('option value', 'usces'); ?>:<input name="search[option_word][0]" type="text" value="<?php echo esc_attr($arr_search['option_word'][0]); ?>" class="text" maxlength="50" />';
		...
		change_product_search_field_1 :function (){
			...
			if( column == 'item_option' ) {
				...
				html = '<?php _e('option name', 'usces'); ?>:<input name="search[product_word][1]" type="text" value="<?php echo esc_attr($arr_search['product_word'][1]); ?>" class="text" maxlength="50" /> <?php _e('option value', 'usces'); ?>:<input name="search[option_word][1]" type="text" value="<?php echo esc_attr($arr_search['option_word'][1]); ?>" class="text" maxlength="50" />';
...
<?php echo apply_filters('usces_filter_order_list_page_js', '', $DT ); ?>
    ...
    (function setCookie() {
        ...
        if($DT->pageLimit == 'on') {
            $data_cookie['currentPageIds'] = $DT->currentPageIds;
        }
        ?>
        $.cookie('<?php echo "{$DT->table}" . "_path"?>', '<?php echo $usces_admin_path; ?>', { path: "<?php echo $usces_admin_path; ?>", domain: "<?php echo $_SERVER['SERVER_NAME']; ?>"});
        $.cookie('<?php echo "{$DT->table}"?>', '<?php echo str_replace("'", "\'",json_encode($data_cookie)) ; ?>', { path: "<?php echo $usces_admin_path; ?>", domain: "<?php echo $_SERVER['SERVER_NAME']; ?>"});
    })();
