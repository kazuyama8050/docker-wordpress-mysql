...
// WP memory limit.
    ...
    return [
        'environment' => [
            ...
            'number_of_media' => array_sum((array)wp_count_attachments(['image/jpeg', 'image/gif', 'image/png'])),
            'number_of_post' => get_option('posts_per_page'),
            'blog_public' => get_option('blog_public'),
            'thumbnail_size' => ['width' => get_option('thumbnail_size_w'), 'height' => get_option('thumbnail_size_h')],
            'medium_size' => ['width' => get_option('medium_size_w'), 'height' => get_option('medium_size_h')],
            'large_size' => ['width' => get_option('large_size_w'), 'height' => get_option('large_size_h')],
...
function get_total_orders(){
    global $wpdb;
    $sql = "SELECT COUNT(*) as total FROM {$wpdb->prefix}usces_order";
    $result = $wpdb->get_row($sql);
    return $result->total;
}
function get_total_members(){
    global $wpdb;
    $sql = "select count(*) as total from {$wpdb->prefix}usces_member";
    $result = $wpdb->get_row($sql);
    return $result->total;
}
...
function get_welcart_system_information(){
    global $usces, $usces_settings;
    $options = get_option('usces');
    $options_ex  = get_option('usces_ex');
    ...
    return [
        ...
        'number_of_orders' => get_total_orders(),
        'number_of_members' => get_total_members(),
        'settlement_selected' => get_option( 'usces_settlement_selected' ),
        'available_settlement' => get_option( 'usces_available_settlement' )
    ];
}

function generate_system_information_text( $environment, $theme, $active_plugins, $inactive_plugins, $welcart_information, $loaded_extensions, $ini_confs){
    ...
    foreach ($active_plugins as $plugin){
        $data .= esc_html( $plugin['name']) . " : " .  __('by', 'usces') . ' '. esc_html($plugin['author_name']) . ' - ' . esc_html($plugin['version']) .
            (($plugin['version'] != $plugin['version_latest']) ? '('.__('update to version', 'usces') . ' ' . esc_html($plugin['version_latest']). ' ' . __('is available', 'usces') . ')' : "") . " \r\n";
    }
    ...
    $data .= "\r\n". '### ' . __('Member email settings', 'usces') . " ### \r\n\r\n";
    ...
    $data .= __( 'Product sub-image rule', 'usces' ) . " : " . (($welcart_information['subimage_rule']) ?  __("apply the new rule<br />(Tow underscores between the Product Code and serial number)", 'usces') : __("not apply the new rule<br />(Truncation Product Code)", 'usces')) . " \r\n";
    ...
    $data .= __( 'Character code in the CSV file', 'usces' ) . " : " . (($welcart_information['csv_encode_type']) ?  __('UTF-8', 'usces') : __('Sift-JIS', 'usces')). " \r\n";
    ...
    $data .= __( 'Ganbare Tencho', 'usces' ) . " : " . (($welcart_information['ganbare_activate_flag']) ?  __('enable', 'usces') : __('disable', 'usces')) . " \r\n";
    $data .= __( 'Linkage Order Upadate', 'usces' ) . " : " . (($welcart_information['stocklink_orderedit_flag']) ?  __('enable', 'usces') : __('disable', 'usces')) . " \r\n";
    $data .= __( 'Linkage Order Collective Upadate', 'usces' ) . " : " . (($welcart_information['stocklink_collective_flag']) ?  __('enable', 'usces') : __('disable', 'usces')) . " \r\n";
    $data .= __( '[New] Order List', 'usces' ) . " : " . (($welcart_information['datalistup_orderlist_flag']) ?  __('enable', 'usces') : __('disable', 'usces')) . " \r\n";
    $data .= __( '[New] Member List', 'usces' ) . " : " . (($welcart_information['datalistup_memberlist_flag']) ?  __('enable', 'usces') : __('disable', 'usces')) . " \r\n";
    $data .= __( 'Verify New Member Email', 'usces' ) . " : " . (($welcart_information['verifyemail_switch_flag']) ?  __('enable', 'usces') : __('disable', 'usces')) . " \r\n";
...
function usces_download_system_information(){
	...
	$data = generate_system_information_text( $environment, $theme, $active_plugins, $inactive_plugins, $welcart_information, $loaded_extensions, $ini_confs);
	$result = array(
		'status' => true,
		'data' => $data,
		'filename' => $filename
