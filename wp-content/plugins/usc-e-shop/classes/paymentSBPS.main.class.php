...
class SBPS_MAIN {
		...
		if ( isset( $_POST['res_result'] ) && isset( $_POST['res_pay_method'] ) && isset( $_POST['order_id'] ) ) {
			...
			if ( empty( $datas['sesid'] ) ) {
				if ( isset( $post_data['free1'] ) ) {
					$acting = substr( $post_data['free1'], 7 );
				} else {
					$acting = $this->paymod_id;
				}
				$log = array( 'acting' => $acting, 'key' => $rand, 'result' => 'Session empty', 'data' => $post_data );
				usces_save_order_acting_error( $log );
				usces_log( $this->acting_name . ' construct : Session empty', 'acting_transaction.log' );
			} else {
	 ...
	 */
	public function set_free_csv( $customer ) {
		$free_csv = '';
	...
	<sps_hashcode>' . $sps_hashcode . '</sps_hashcode>
			...
			if ( empty( $xml_settlement ) ) {
				...
				$result_data = array(
					'acting' => $this->acting_card,
					'acting_return' => 0,
					'res_err_code' => $request_settlement['res_err_code'],
				);
	...
	<sps_hashcode>' . $sps_hashcode . '</sps_hashcode>
				...
				if ( empty( $xml_credit ) ) {
					...
					$result_data = array(
						'acting' => $this->acting_card,
						'acting_return' => 0,
						'res_err_code' => $request_credit['res_err_code'],
					);
					wp_redirect( add_query_arg( $result_data, USCES_CART_URL ) );
					exit;
				}
...
<sps-api-request id="ST02-00201-101">
	...
	<sps_hashcode>' . $sps_hashcode . '</sps_hashcode>
						...
						if ( empty( $xml_sales ) ) {
							...
							$result_data = array(
								'acting' => $this->acting_card,
								'acting_return' => 0,
								'res_err_code' => $request_sales['res_err_code'],
							);
							wp_redirect( add_query_arg( $result_data, USCES_CART_URL ) );
							exit;
						}
						...
						if ( isset( $response_sales['res_result'] ) && 'OK' === $response_sales['res_result'] ) {
							...
							$result_data = array(
								'acting' => $this->acting_card,
								'acting_return' => 0,
								'res_err_code' => $response_sales['res_err_code'],
							);
							wp_redirect( add_query_arg( $result_data, USCES_CART_URL ) );
							exit;
						}
					...
					$result_data = array(
						'acting' => $this->acting_card,
						'acting_return' => 0,
						'res_err_code' => $response_credit['res_err_code'],
					);
					wp_redirect( add_query_arg( $result_data, USCES_CART_URL ) );
					exit;
				}
				...
				$result_data = array(
					'acting' => $this->acting_card,
					'acting_return' => 0,
					'res_err_code' => $response_settlement['res_err_code'],
				);
				wp_redirect( add_query_arg( $result_data, USCES_CART_URL ) );
				exit;
			}
	 ...
	 * @return array
	 */
	protected function xml2assoc( $xml_data, $encrypted_flg = 0 ) {
		$object_data = simplexml_load_string( $xml_data );
		$json_data = json_encode( $object_data );
		$array_data = json_decode( $json_data, true );
		if ( 1 == $encrypted_flg ) {
			$acting_opts = $this->get_acting_settings();
			if ( isset( $array_data['res_pay_method_info'] ) ) {
			...
			if ( isset( $array_data['pay_method_info'] ) ) {
