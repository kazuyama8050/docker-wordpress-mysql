...
class ESCOTT_MAIN
{
	 ...
	 *
	 * @param  array $data
	 * @param  array $member_metas
	 * @param  array $usces_member_history
	 */
...
<?php
			...
			$log = array( 'acting'=>$this->acting_card.'(3ds_process)', 'key'=>$results['MerchantFree1'], 'result'=>'SESSION ERROR', 'data'=>$results );
			usces_save_order_acting_error( $log );
			$result_data = array(
				'MerchantFree2' => $results['MerchantFree2'],
				'ResponseCd' => 'NG',
...
<?php
	...
	public function pre_purchase() {
		...
		if( 'on' == $acting_opts['sec3d_activate'] && isset( $post_data['done3d'] ) && isset( $post_data['rand'] ) ) {
	...
	public function acting_processing( $acting_flg, $post_query ) {
		...
		if( !$usces_entries || !$cart ) {
			if( 'on' == $acting_opts['sec3d_activate'] && isset( $post_data['done3d'] ) ) {
				...
				$cart = $usces->cart->get_cart();
				if( ! $usces_entries || ! $cart ) {
					$log = array( 'acting'=>$this->acting_card.'(3ds_process)', 'key'=>$post_data['rand'], 'result'=>'SESSION RESULT ERROR', 'data'=>$post_data );
					usces_save_order_acting_error( $log );
					$result_data = array(
				...
				$log = array( 'acting'=>$this->acting_card, 'key'=>$post_data['rand'], 'result'=>'SESSION ERROR', 'data'=>$post_data );
				usces_save_order_acting_error( $log );
				$result_data = array(
					'MerchantFree2' => $acting_flg,
					'ResponseCd' => 'NG',
		...
		if( $this->acting_flg_card == $acting_flg ) {
			if( isset( $_REQUEST['MerchantFree1'] ) && usces_get_order_id_by_trans_id( (int)$_REQUEST['MerchantFree1'] ) ) {
				...
				if( 0 < count( $error_message ) ) {
					...
					if( 1 == count( $responsecd ) && $responsecd[0] == 'NG' ) {
					} else {
						$html .= '
...
<?php
	 ...
	 * @param  float $cod_fee
	 * @param  array $usces_entries
	 * @param  float $total_items_price
	 * @param  int   $use_point
	 * @param  float $discount
	 * @param  float $shipping_charge
	 * @param  float $amount_by_cod
	 * @return float
	 */
	 ...
	 *
	 * @param  int    $member_id
	 * @param  string $KaiinId
	 * @param  string $KaiinPass
	 * @return array
	 */
