...
class ZEUS_SETTLEMENT
{
	...
	public function admin_notices() {
		...
		if( isset( $acting_opts['activate'] ) && 'on' == $acting_opts['activate'] ) {
			...
			if( !isset( $acting_opts['vercheck'] ) || '115' != $acting_opts['vercheck'] ) {
				echo '<div class="error"><p>決済に「ゼウス」をご利用の場合は、<a href="'.admin_url( 'admin.php?usces_page=usces_settlement' ).'">「セキュリティーコード」の設定内容を確認</a>して更新ボタンを押してください。設定を更新するとこのメッセージは表示されなくなります。</p></div>';
			}
		}
	}
	...
	public function admin_scripts() {
		...
		$admin_page = ( isset( $_GET['usces_page'] ) ) ? $_GET['usces_page'] : '';
		switch( $admin_page ):
...
jQuery( document ).ready( function( $ ) {
	if( '2' == $( "input[name='connection']:checked" ).val() ) {
		...
		$( "#3dsecur_zeus_2" ).prop( "checked", true );
	}
	...
	if( '1' == $( "input[name='3dsecur']:checked" ).val() ) {
		$( "#connection_zeus_2" ).prop( "checked", true );
		$( ".authkey_zeus" ).css( "display", "" );
		$( ".3dsecur_zeus" ).css( "display", "" );
	}
...
<?php
	...
	/**
	 * 決済オプション登録・更新
	 * @hook   usces_action_admin_settlement_update
	 * @param  -
	 * @return -
	 */
	...
	/**
	 * クレジット決済設定画面タブ
	 * @hook   usces_action_settlement_tab_title
	 * @param  -
	 * @return -
	 * @echo   html
	 */
	...
	/**
	 * クレジット決済設定画面フォーム
	 * @hook   usces_action_settlement_tab_body
	 * @param  -
	 * @return -
	 * @echo   html
	 */
...
<?php
	...
	/**
	 * 併用不可決済モジュール
	 * @hook   usces_filter_unavailable_payments
	 * @param  -
	 * @return -
	 */
	...
	/**
	 * 管理画面送信メール
	 * @hook   usces_filter_order_confirm_mail_payment
	...
	/**
	 * ポイント即時付与
	 * @hook   usces_filter_is_complete_settlement
	...
	/**
	 * 購入完了メッセージ
	 * @hook   usces_filter_completion_settlement_message
	...
	/**
	 * 決済リンクキー
	 * @hook   usces_filter_get_link_key
	...
	/**
	 * 受注データ復旧処理
	 * @hook   usces_action_revival_order_data
	...
	/**
	 * 受注データから取得する決済情報のキー
	 * @hook   usces_filter_settle_info_field_meta_keys
	 * @param  $keys
	 * @return array $keys
	 */
	...
	/**
	 * 受注編集画面に表示する決済情報のキー
	 * @hook   usces_filter_settle_info_field_keys
	 * @param  $keys
	 * @return array $keys
	 */
	...
	/**
	 * 受注編集画面に表示する決済情報の値整形
	 * @hook   usces_filter_settle_info_field_value
	 ...
	 * @hook   usces_action_admin_member_info
...
<?php			endif;
	 ...
	 * @hook   usces_action_post_update_memberdata
	 * @param  $member_id $res
	 * @return -
	 */
	...
	/**
	 * 支払方法説明
	 * @hook   usces_filter_payment_detail
	 ...
	 * @hook   usces_filter_payments_str
	 ...
	 * @hook   usces_filter_payments_arr
	...
	/**
	 * カード情報入力チェック
	 * @hook   usces_filter_delivery_check
	 * @param  $mes
	 * @return string $mes
	 */
	...
	/**
	 * 支払方法ページ用入力フォーム
	 * @hook   usces_filter_delivery_secure_form_loop
	 * @param  $nouse $payment
	 * @return string $html
	 */
	public function delivery_secure_form_loop( $nouse, $payment ) {
		...
		case 'acting_zeus_card':
			...
			$html_howpay = '';
			$member_page = ( isset( $_GET['usces_page'] ) && 'member_update_settlement' == $_GET['usces_page'] ) ? true : false;
	...
	/**
	 * 内容確認ページ [注文する] ボタン
	 * @hook   usces_filter_confirm_inform
	...
	/**
	 * 内容確認ページ ポイントフォーム
	 * @hook   usces_action_confirm_page_point_inform
	 * @param  -
	 * @return -
	 * @echo point_inform()
	 */
	...
	/**
	 * 内容確認ページ ポイントフォーム
	 * @hook   usces_filter_confirm_point_inform
	 * @param  $html
	 * @return string $html
	 */
	...
	/**
	 * 決済処理
	 * @hook   usces_action_acting_processing
	...
	public function acting_processing( $acting_flg, $post_query ) {
		...
		if( empty($cart) ) {
			...
			if ( isset( $_REQUEST['sendpoint'] ) ) {
				usces_restore_order_acting_data( $_REQUEST['sendpoint'] );
				usces_log( 'zeus restore entry data (acting_processing) : '.print_r( $_REQUEST, true ), 'acting_transaction.log' );
				$cart = $usces->cart->get_cart();
				if ( empty($cart) ) {
			...
			if( $fp ) {
				...
				if( false !== strpos( $page, 'Success_order' ) ) {
					...
					$result_data = array(
						...
						'acting_return' => '1',
						'order_number' => $ordd,
						'wctid' => $_POST['sendpoint']
					);
					if( 'on' == $acting_opts['howpay'] && isset( $_POST['howpay'] ) && WCUtils::is_zero( $_POST['howpay'] ) ) {
						$result_data = array_merge( $result_data, array(
							'howpay' => $_POST['howpay'],
							'div' => $_POST['div']
						) );
					}
	...
	/**
	 * 
	 * @hook   usces_pre_acting_return
	 */
	public function pre_acting_return() {
		global $usces;

		$rand = isset( $_GET['wctid'] ) ? $_GET['wctid'] : '';
		if( !$rand ){
			$rand = isset( $_GET['sendpoint'] ) ? $_GET['sendpoint'] : '';
		}
		if( $rand ) {
			$cart = $usces->cart->get_cart();
			if( empty( $cart ) ) {
				usces_restore_order_acting_data( $rand );
				usces_log( 'zeus restore entry data (acting_return) : '.print_r( $_GET, true ), 'acting_transaction.log' );
			}
		}
	}
	...
	public function pre_purchase() {
		...
		$cart = $usces->cart->get_cart();
		if( empty( $cart ) ) {
			usces_restore_order_acting_data( $_GET['sendpoint'] );
			usces_log( 'zeus restore entry data (purchase) : '.print_r( $_GET, true ), 'acting_transaction.log' );
		}
	}

	/**
	 * 決済完了ページ制御
	 * @hook   usces_filter_check_acting_return_results
	 * @param  $results
	...
	/**
	 * 重複オーダー禁止処理
	 * @hook   usces_filter_check_acting_return_duplicate
	 ...
	 * @hook   usces_action_reg_orderdata
	 * @param  $args = array(
	...
	/**
	 * 決済エラーメッセージ
	 * @hook   usces_filter_get_error_settlement
	 * @param  $html
	 * @return string $html
	 */
	...
	public function set_uscesL10n( $l10n, $post_id ) {
		...
		if( 'delivery' == $usces->page ) {
			...
			if( !empty( $pcid ) && !empty( $partofcard ) ) {
				$member_update_settlement = add_query_arg( array( 'usces_page'=>'member_update_settlement', 're-enter'=>1 ), USCES_MEMBER_URL );
...
<?php
	...
	/**
	 * 利用可能な決済モジュール
	 * @hook   usces_filter_available_payment_method
	 * @param  $payments
	...
	/**
	 * クレジットカード登録・変更ページ表示
	 * @hook   usces_filter_template_redirect
	 * @param  -
	 * @return -
	 */
	...
	/**
	 * 会員データ削除チェック
	 * @hook   usces_filter_delete_member_check
	 * @param  $del $member_id
	 * @return boolean $del
	 */
	...
	/**
	 * クレジットカード登録・変更ページリンク
	 * @hook   usces_action_member_submenu_list
	 * @param  -
	 * @return -
	...
	/**
	 * クレジットカード登録・変更ページリンク
	 * @hook   usces_filter_member_submenu_list
	 * @param  $html $member
	 * @return string $html
	 */
	...
	public function member_update_settlement_form() {
		...
		$message = '';
		$html = '';
		$update_settlement_url = add_query_arg( array( 'usces_page' => 'member_update_settlement', 'settlement' => 1, 're-enter' => 1 ), USCES_MEMBER_URL );
		...
		if( 'outside' == $EnrolRes['response']['result']['status'] ) {
			...
			if( 'success' == $PayRes['response']['result']['status'] ) {
				$result_data = array(
					'acting' => 'zeus_card',
					'acting_return' => '1',
					'zeussuffix' => $PayRes['response']['card']['number']['suffix'],
					'zeusordd' => $PayRes['response']['order_number'],
					'wctid' => $sendpoint
				);
				wp_redirect( add_query_arg( $result_data, USCES_CART_URL ) );
				exit;
				...
				$result_data = array(
					'acting' => 'zeus_card',
					'acting_return' => '0',
					'status' => $PayRes['response']['result']['status'],
					'code' => $PayRes['response']['result']['code']
				);
				wp_redirect( add_query_arg( $result_data, USCES_CART_URL ) );
				exit;
			}
			...
			$result_data = array(
				'acting' => 'zeus_card',
				'acting_return' => '0',
				'status' => $EnrolRes['response']['result']['status'],
				'code' => $EnrolRes['response']['result']['code']
			);
			wp_redirect( add_query_arg( $result_data, USCES_CART_URL ) );
			exit;
		}
	...
	protected function zeus_3dsecure_auth() {
		...
		if( 'success' != $AuthRes['response']['result']['status'] ) {
			...
			$result_data = array(
				'acting' => 'zeus_card',
				'acting_return' => '0',
				'status' => $AuthRes['response']['result']['status'],
				'code' => $AuthRes['response']['result']['code']
			);
			wp_redirect( add_query_arg( $result_data, USCES_CART_URL ) );
			exit;
		}
