...
<?php
	...
	public function settlement_update() {
		...
		$options['acting_settings']['welcart']['card_key_aes'] = ( isset( $_POST['card_key_aes'] ) ) ? $_POST['card_key_aes'] : '';
		$options['acting_settings']['welcart']['card_key_iv'] = ( isset( $_POST['card_key_iv'] ) ) ? $_POST['card_key_iv'] : '';
...
?>
	...
	<form action="" method="post" name="welcart_form" id="welcart_form">
		...
		<table class="settle_table">
			...
			<tr id="ex_sec3d_activate" class="explanation card_welcart card_sec3d_welcart"><td colspan="2"><?php _e( "3D secure authentication at the time of payment. If you want to use it, you need to apply to Sony Payment Service Co., Ltd.", 'usces' ); ?></td></tr>
			<tr class="card_welcart card_key_welcart">
				<th><a class="explanation-label" id="label_ex_card_key_aes_welcart"><?php _e( 'Encryption Key', 'usces' );//暗号化キー ?></a></th>
				<td><input name="card_key_aes" type="text" id="card_key_aes_welcart" value="<?php echo $acting_opts['card_key_aes']; ?>" class="regular-text" /></td>
			</tr>
			<tr id="ex_card_key_aes_welcart" class="explanation card_welcart card_key_welcart"><td colspan="2"><?php _e( 'Encryption key (single-byte alphanumeric characters only) issued by e-SCOTT.', 'usces' ); ?><?php _e( 'If you want to use 3D Secure Authentication or External Link Type Payment, please apply to Sony Payment Service Co., Ltd.', 'usces' ); ?></td></tr>
			<tr class="card_welcart card_key_welcart">
				<th><a class="explanation-label" id="label_ex_card_key_iv_welcart"><?php _e( 'Initialization Vector', 'usces' );//初期化ベクトル ?></a></th>
				<td><input name="card_key_iv" type="text" id="card_key_iv_welcart" value="<?php echo $acting_opts['card_key_iv']; ?>" class="regular-text" /></td>
			</tr>
			<tr id="ex_card_key_iv_welcart" class="explanation card_welcart card_key_welcart"><td colspan="2"><?php _e( 'Initialization vector (single-byte alphanumeric characters only) issued by e-SCOTT.', 'usces' ); ?><?php _e( 'If you want to use 3D Secure Authentication or External Link Type Payment, please apply to Sony Payment Service Co., Ltd.', 'usces' ); ?></td></tr>
		...
		<table class="settle_table">
			...
			<tr class="unionpay_welcart">
				<th><a class="explanation-label" id="label_ex_unionpay_key_aes_welcart"><?php _e( 'Encryption Key', 'usces' );//銀聯暗号化キー ?></a></th>
			...
			<tr id="ex_unionpay_key_aes_welcart" class="explanation unionpay_welcart"><td colspan="2"><?php _e( 'UnionPay encryption key (single-byte alphanumeric characters only) issued by e-SCOTT.', 'usces' ); ?></td></tr>
			<tr class="unionpay_welcart">
				<th><a class="explanation-label" id="label_ex_unionpay_key_iv_welcart"><?php _e( 'Initialization Vector', 'usces' );//銀聯初期化ベクトル ?></a></th>
			...
			<tr id="ex_unionpay_key_iv_welcart" class="explanation unionpay_welcart"><td colspan="2"><?php _e( 'UnionPay initialization vector (single-byte alphanumeric characters only) issued by e-SCOTT.', 'usces' ); ?></td></tr>
			<tr class="unionpay_welcart">
						...
						if( 'OK' == $response_data['ResponseCd'] ) {
							...
							if( !empty( $order_data ) ) {
								...
								if( 'ordercompletion' == $res ) {
									...
									wp_redirect( add_query_arg( array( 'acting'=>$this->acting_unionpay, 'acting_return'=>1, 'result'=>1, '_nonce'=>$_nonce ), USCES_CART_URL ) );
								} else {
									$logdata = array_merge( $usces_entries['order'], $response_data );
									$log = array( 'acting'=>$this->acting_unionpay, 'key'=>$response_data['MerchantFree1'], 'result'=>'ORDER DATA REGISTERED ERROR', 'data'=>$logdata );
								...
								$log = array( 'acting'=>$this->acting_unionpay, 'key'=>$response_data['MerchantFree1'], 'result'=>'ORDER DATA RESTORE ERROR', 'data'=>$response_data );
								usces_save_order_acting_error( $log );
								wp_redirect( add_query_arg( array( 'acting'=>$this->acting_unionpay, 'acting_return'=>0, 'result'=>0 ), USCES_CART_URL ) );
							}
							...
							$log = array( 'acting'=>$this->acting_unionpay, 'key'=>$response_data['MerchantFree1'], 'result'=>$response_data['ResponseCd'], 'data'=>$response_data );
							usces_save_order_acting_error( $log );
							wp_redirect( add_query_arg( array( 'acting'=>$this->acting_unionpay, 'acting_return'=>0, 'result'=>0 ), USCES_CART_URL ) );
						}
					}
					...
					if( isset( $response_data['OperateId'] ) && isset( $response_data['ResponseCd'] ) && ( isset( $response_data['MerchantId'] ) && $acting_opts['merchant_id'] == $response_data['MerchantId'] ) ) {
						...
						if( isset( $response_data['MerchantFree1'] ) && ( isset( $response_data['MerchantFree2'] ) && 'acting_welcart_card' == $response_data['MerchantFree2'] ) ) {
							if( 'OK' == $response_data['ResponseCd'] ) {
									...
									if( 'OK' == $response_data['ResponseCd'] ) {
										...
										if( !empty( $order_data ) ) {
											...
											if( 'ordercompletion' == $res ) {
												...
												wp_redirect( add_query_arg( array( 'acting'=>$this->acting_card, 'acting_return'=>1, 'result'=>1, '_nonce'=>$_nonce ), USCES_CART_URL ) );
											} else {
												$logdata = array_merge( $usces_entries['order'], $response_data );
											...
											$log = array( 'acting'=>$this->acting_card, 'key'=>$response_data['MerchantFree1'], 'result'=>'ORDER DATA RESTORE ERROR', 'data'=>$response_data );
											usces_save_order_acting_error( $log );
											wp_redirect( add_query_arg( array( 'acting'=>$this->acting_card, 'acting_return'=>0, 'result'=>0 ), USCES_CART_URL ) );
										}
									} else {
								...
								$log = array( 'acting'=>$this->acting_card, 'key'=>$response_data['MerchantFree1'], 'result'=>$response_data['ResponseCd'], 'data'=>$response_data );
								usces_save_order_acting_error( $log );
								wp_redirect( add_query_arg( array( 'acting'=>$this->acting_card, 'acting_return'=>0, 'result'=>0 ), USCES_CART_URL ) );
							}
...
/*			if( usces_is_login() && 'on' == $acting_opts['quickpay'] && empty( $quick_member ) ) {
				...
				if( 'OK' == $response_data['ResponseCd'] ) {
					$res = $usces->order_processing( $response_data );
					if( 'ordercompletion' == $res ) {
						wp_redirect( add_query_arg( array( 'acting'=>$this->acting_card, 'acting_return'=>1, 'result'=>1, '_nonce'=>wp_create_nonce( $this->acting_flg_card ) ), USCES_CART_URL ) );
					} else {
						$logdata = array_merge( $usces_entries['order'], $response_data );
						$log = array( 'acting'=>$this->acting_card, 'key'=>$rand, 'result'=>'ORDER DATA REGISTERED ERROR', 'data'=>$logdata );
						usces_save_order_acting_error( $log );
...
/*			}*/
	...
	public function footer_scripts() {
		...
		//クレジットカード情報更新ページ
		if( isset( $_GET['usces_page'] ) && ( 'member_register_settlement' == $_GET['usces_page'] || 'member_update_settlement' == $_GET['usces_page'] ) ) {
	...
	public function delivery_secure_form_howpay( $html ) {
		...
		if( isset( $_GET['usces_page'] ) && ( 'member_update_settlement' == $_GET['usces_page'] || 'member_register_settlement' == $_GET['usces_page'] ) ) {
			$html = '';
		}
		return $html;
	}
...
<?php
	...
	public function set_uscesL10n( $l10n, $post_id ) {
		...
		if( $usces->is_member_page( $_SERVER['REQUEST_URI'] ) && ( isset( $_GET['usces_page'] ) && ( 'member_register_settlement' == $_GET['usces_page'] || 'member_update_settlement' == $_GET['usces_page'] ) ) ) {
			$acting_opts = $this->get_acting_settings();
			if( isset( $acting_opts['card_activate'] ) && 'token' == $acting_opts['card_activate'] ) {
	...
	public function update_settlement( $html, $member ) {
		...
		if( 'on' == $acting_opts['quickpay'] ) {
			...
			if( 'OK' == $response_member['ResponseCd'] ) {
				$update_settlement_url = add_query_arg( array( 'usces_page'=>'member_update_settlement', 're-enter'=>1 ), USCES_MEMBER_URL );
				$html .= '
				<div class="gotoedit">
				<a href="'.$update_settlement_url.'">'.__( "Change the credit card is here >>", 'usces' ).'</a>
				</div>';
			} else {
				$register_settlement_url = add_query_arg( array( 'usces_page'=>'member_register_settlement', 're-enter'=>1 ), USCES_MEMBER_URL );
				$html .= '
				<div class="gotoedit">
	...
	public function member_update_settlement_form() {
		...
		//マイページ
			...
			$update_settlement_url = add_query_arg( array( 'usces_page'=>$usces->page, 'settlement'=>1, 're-enter'=>1 ), USCES_MEMBER_URL );
			if( '' != $done_message ) {
				$script .= '
				<script type="text/javascript">
...
/*
		$( document ).on( "click", "#dl_atodene_update_csv", function() {
			...
			location.href = "'.USCES_ADMIN_URL.'?page=usces_orderlist&order_action=download_atodene_update"+listcheck+"&noheader=true&_nonce='.wp_create_nonce( 'csv_nonce' ).'";
		});
*/
		$html .= '
	 ...
	 *
	 * @param  array  $payments_restriction
	 * @param  string $payments_value
	 * @return array
	 */
	function payment_method_restriction_atodene( $payments_restriction, $payments_value ) {
		global $usces;
